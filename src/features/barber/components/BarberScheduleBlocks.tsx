...
export const BarberScheduleBlocks: React.FC<Props> = ({ barberId, showFeedback }) => {
  ...
  const ownTimedBlocks = useMemo(() => scheduleBlocks
    .filter(block => block.barberId === barberId && block.type === 'block')
    .sort((a, b) => `${b.date} ${b.startTime}`.localeCompare(`${a.date} ${a.startTime}`)), [barberId, scheduleBlocks]);
